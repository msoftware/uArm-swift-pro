...
def sorti():
    for i in range(len(x_liste)):
        swift.set_position(x_liste[i], y_liste[i], 60, speed = 20000, relative=False, wait = True)
        swift.set_position(z = 60, speed = 20000, wait = True)
        r = 60
        while r > -5:
            r = r - 1
        ...
        if r_liste[i] in range(10, 20):
            swift.set_position(210, 208, 30, speed = 20000, wait = True) 

        elif r_liste[i] in range(20, 30):
            swift.set_position(170, 208, 30, speed = 20000, wait = True) 
            
        elif r_liste[i] in range(31, 80):
            swift.set_position(210, -210, 60, speed = 20000, wait = True) 
        else:   
            swift.set_position(250, -210, 20, speed = 20000, relative=False, wait = True) 
...
def video_loop():
     ...
     if ok:  
        ...
        rows = gray.shape[0]
        circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1.1, rows / 12,
        ...
        if circles is not None:
            ...
            for i in range(len(circles)):
                for (x, y, r) in circles:
                    ...
                    uarm_x = int(cam_y[circles[i][1]])
                    uarm_y = int(cam_x[circles[i][0]])
                    uarm_r = circles[i][2]
                x_liste.append(uarm_x)
                y_liste.append(uarm_y)
                r_liste.append(uarm_r)
...
main.title("uArm sort by diameter")   
...
mSpei.add_command(label="Kamera", command = kamm)
mSpei.add_separator()
